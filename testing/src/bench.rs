...
pub fn run_bench(cores: &[CoreId], clock: &Clock, args: &CliArgs, bench: impl Bench) {
    ...
    if args.csv {
        ...
        // Header row with destination core ids
        let mut header = Vec::with_capacity(1 + cores.len());
        header.push(String::new());
        header.extend(cores.iter().map(|c| c.id.to_string()));
        println!("{}", header.join(","));

        for (i, row) in results.rows().into_iter().enumerate() {
            let mut values = Vec::with_capacity(1 + row.len());
            // First column is the source core id
            values.push(cores[i].id.to_string());
